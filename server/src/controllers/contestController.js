...
module.exports.setOfferStatus = async (req, res, next) => {
  const {
    body: { offerId, creatorId, contestId, command, orderId, priority },
  } = req;
  let transaction;
  if (command === 'reject') {
    try {
...
module.exports.getCustomersContests = (req, res, next) => {
  ...
  db.Contest.findAll({
    where: { status: status, userId: req.tokenData.userId },
    limit,
...
module.exports.getContests = async (req, res, next) => {
  const {
    query: { typeIndex, contestId,industry,awardSort,limit,offset,ownEntries } } = req;
      ...
      include: [
        {
          ...
          where: ownEntries === 'false' ? { userId: req.tokenData.userId } : {},
          attributes: ['id'],
        },
      ],
    });
